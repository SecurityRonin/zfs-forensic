...
- **Tier-1 validated (full read path)** — the *entire* reader path (label → uberblock → MOS → DSL → ZPL → SA → **file content**) is checked against a real, vendor-authored **FreeBSD 14.3 ZFS-root** image whose pool is a single `disk` vdev (no RAIDZ). `zpl_read_path` reads genuine third-party file bytes whose sha256 matches two independent OpenZFS oracles — `zdb` and a live read-only kernel `zfs mount`.
- **Tier-1 validated (bootstrap layer)** — the vdev label + nvlist config + uberblock reader is additionally checked against the OpenZFS project's own `zol-0.6.1` reference pool (`openzfs/zfs-images`), a third-party artifact whose ground truth comes from `zdb -l` / `zdb -u`, a wholly separate implementation.
- **Tier-2 regression corpus** — always-on, byte-exact self-mint fixtures (`tpool` / `dtpool`, `zdb` as the independent oracle) pin each layer's decode as a fast CI backstop under the Tier-1 gates. Reading the raidz `zol-0.6.1` pool's data layers still needs RAIDZ parity reconstruction (deferred); the FreeBSD single-vdev pool is what carries those layers to Tier-1. See [`docs/validation.md`](https://securityronin.github.io/zfs-forensic/validation/).
